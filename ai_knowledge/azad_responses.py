...
from .company_info import get_welcome_message  # noqa: E402
from .system_knowledge import ALL_MODULES, get_module_help, search_knowledge  # noqa: E402,F401
from .tax_customs_knowledge import TAX_CUSTOMS_GUIDE, get_tax_info, get_customs_info  # noqa: E402,F401
from .parts_knowledge import PARTS_DATABASE, get_part_info, search_parts  # noqa: E402,F401
from .user_guide import USER_GUIDE, get_guide, get_help_for_task  # noqa: E402,F401
from .analytics_predictions import ANALYTICS_LIBRARY, SalesAnalytics, InventoryAnalytics  # noqa: E402,F401
from .knowledge_sources import knowledge_manager, KNOWLEDGE_SOURCES, recommend_sources_for_query, SOURCES_GUIDE  # noqa: E402,F401
from .dialects import dialect_manager, apply_dialect, get_dialectal_greeting  # noqa: E402,F401
from .beginners_mode import beginners_guide, BEGINNERS_TUTORIALS  # noqa: E402
from .tax_system import get_tax_advice  # noqa: E402
from .customs import get_customs_advice  # noqa: E402,F401
from .customer_service import get_customer_service_tip  # noqa: E402
from .system_guide import get_system_guide  # noqa: E402
from .market_insights import get_market_insights  # noqa: E402
from .learning_system import learning_system  # noqa: E402
from .global_knowledge import global_connector, expertise_updater  # noqa: E402,F401
from .self_improvement import self_improvement  # noqa: E402
from .system_integration import system_integrator  # noqa: E402
from .data_analyzer import data_analyzer  # noqa: E402
from .knowledge_expansion import knowledge_expander  # noqa: E402
from .azad_personality import azad_personality  # noqa: E402
from .security_rules import security_rules  # noqa: E402,F401
from .document_generator import document_generator  # noqa: E402
from .advanced_laws import advanced_laws  # noqa: E402,F401
from .context_engine import context_engine  # noqa: E402,F401
from .semantic_matcher import semantic_matcher, understand_message  # 🚀 النظام الذكي الجديد!  # noqa: E402,F401
from .intelligent_assistant import intelligent_assistant  # 🧠 الذكاء الحقيقي!  # noqa: E402
...
class AzadResponses:
    ...
    @staticmethod
    def smart_response(message, context=None):  # noqa: C901
        ...
        _ = context.get('is_owner', False) if context else False
...
أو: http://localhost:5000/ai/config"""
        ...
        # هل أنت محلي/جروك؟ | حالة النظام | وضع AI
            ...
            if has_api_key and provider:
                status_msg = """🤖 **حالة أزاد - نظام هجين نشط**
...
⚠️ **تنبيه**: هذه المعلومات سرية ومتاحة لك فقط كمالك للنظام."""
        ...
        elif any(kw in msg_lower for kw in ['مرحبا', 'أهلا', 'السلام', 'hello', 'hi', 'مرحبتين', 'أهلين', 'هلا', 'إيش', 'شلون']) or msg_lower in ['أزاد', 'azad', 'مساعد', 'assistant']:  # noqa: E501
...
💡 **نصيحة:** أضف بيانات لأحصل على رؤى أعمق!"""
        ...
        # توقعات متقدمة
            ...
            try:
                ...
                # التنبؤ
                _ = SalesAnalytics.predict_next_month_sales(historical)

                return """🔮 **توقع مبيعات الشهر القادم:**
...
💡 **النصيحة:** {'ممتاز!' if overall['margin'] >= 25 else 'راجع التسعير'}"""
        ...
        # التقارير السريعة
        elif any(kw in msg_lower for kw in ['تقرير', 'report']) and any(kw in msg_lower for kw in ['مبيعات', 'sales', 'مشتريات', 'purchases', 'مخزون', 'inventory', 'ذمم', 'receivables']):  # noqa: E501
        ...
        # القوانين الضريبية
        elif any(kw in msg_lower for kw in ['قانون', 'law', 'ضريبة', 'tax', 'ضرائب']) and any(kw in msg_lower for kw in ['فلسطين', 'palestine', 'اسرائيل', 'israel', 'خليج', 'gulf']):  # noqa: E501
            return AzadResponses._handle_tax_laws_query(message)

        # قوانين الشحن والتخليص
        elif any(kw in msg_lower for kw in ['شحن', 'shipping', 'تخليص', 'customs', 'جمارك']) and any(kw in msg_lower for kw in ['قانون', 'law', 'إجراءات', 'procedures']):  # noqa: E501
...
**اسألني أي شيء - أحلل بياناتك الحقيقية! 🧠**"""
    ...
    def _get_improvement_response(message):
        ...
        if 'تلقائي' in message_lower or 'automatic' in message_lower:
            ...
            _ = self_improvement.auto_improve()
            return """🚀 تم تطبيق التحسين التلقائي!
...
🎯 أزاد يتطور باستمرار ليصبح أفضل مساعد في العالم!"""
            ...
            # عرض الأهداف الحالية
            _ = self_improvement.track_progress()
            return """🎯 أهداف أزاد الحالية:
...
أزاد يسعى للوصول لمستوى خبير عالمي!"""
            ...
            # عرض حالة التحسين
            _ = self_improvement.get_improvement_status()
            return """🔧 حالة التحسين الذاتي:
...
🚀 أزاد يتطور ذاتياً ليصبح أفضل مساعد في العالم!"""
    ...
    def _get_status_response():
        ...
        # تحليل الأداء
        _ = self_improvement.analyze_performance()

        # رؤى التعلم
        _ = learning_system.get_learning_insights()

        # التطور
        _ = self_improvement.evolve_capabilities()

        return """📊 تقرير حالة أزاد الشامل:
...
أزاد في طريقه ليصبح خبيراً عالمياً! 🌍"""
    ...
    def _handle_customer_balance_query(message):
        ...
        if debt_analysis['success']:
            _ = debt_analysis['debt_analysis']
            response = """💰 **رصيد العميل: {customer['name']}**
...
• آخر مبيعة: {customer['last_sale_date'] or 'لا توجد'}"""
    ...
    def _handle_customer_info_query(message):
        ...
        if sales_summary['success']:
            _ = sales_summary['summary']
            response = """👤 **بيانات العميل: {customer['name']}**
...
💸 **الرصيد الحالي: {customer['balance_aed']:,.2f} AED**"""
    ...
    def _handle_product_stock_query(message):
        ...
        if product['current_stock'] == 0:
            ...
            _ = "🟢"
            _ = "مخزون جيد"
            _ = "لا يحتاج طلب حالياً"

        response = """📦 **مخزون المنتج: {product['name']}**
...
💡 **نصيحة:** {'راجع الموردين للحصول على أفضل الأسعار' if product['current_stock'] <= product.get('alert_limit', 0) else 'المخزون في حالة جيدة'}"""
    ...
    def _handle_system_summary_query():
        ...
        _ = summary['summary']
        _ = financial['financial']

        response = """📊 **ملخص النظام الشامل**
...
أو يمكنك استخدام النموذج في النظام لإضافة العملاء بطريقة أسهل."""
    ...
    def _handle_search_query(message):
        ...
        response = """🔍 **نتائج البحث عن: "{search_term}"**
...
{chr(10).join([f"• {product['name']} (SKU: {product['sku']}) - مخزون: {product['stock']} - سعر: {product['price']:,.2f} AED" for product in results['products'][:5]])}  # noqa: E501
...
💡 **مثال:** "أضف موقع https://tax.gov.ae في فئة الضرائب والجمارك" """
    ...
    def _handle_knowledge_search(message):
        ...
        _ = chr(10).join(results_list)

        response = """نتائج البحث في المعرفة عن: "{search_term}"
...
للمزيد من التفاصيل، يمكنك طلب معلومات أكثر عن أي نتيجة."""
    ...
    @staticmethod
    def _handle_document_generation(message):  # noqa: C901
...
**أزاد خبير في القوانين الإسرائيلية!**"""
            ...
            return """🇦🇪 **القوانين الضريبية الخليجية**
...
**أزاد خبير في قوانين دول الخليج!**"""
    ...
    def _show_knowledge_sources(message):
        ...
        if 'كل' in msg_lower or 'all' in msg_lower:
            ...
            response = """🌐 **مصادر المعرفة المتاحة لأزاد:**
...
**أزاد خبير في معايير الإلكترونيات!**"""
    ...
    def _smart_sales_analysis(context):
        ...
        if avg_7d > avg_30d * Decimal('1.1'):
            _ = f"📈 تحسن ممتاز! المبيعات الأخيرة أعلى من المتوسط بـ {float((avg_7d / avg_30d - 1) * 100):.1f}%"
        elif avg_7d < avg_30d * Decimal('0.9'):
            _ = f"📉 تراجع ملحوظ! المبيعات انخفضت بـ {float((1 - avg_7d / avg_30d) * 100):.1f}%"
        else:
            _ = "➡️ مبيعات مستقرة"

        return """📊 **تحليل المبيعات الذكي:**
...
**التوصية:** {"استمر على هذا النهج!" if avg_7d >= avg_30d else "راجع استراتيجية المبيعات والتسويق"}"""
    ...
    def _quick_invoice_link():
        ...
        _ = azad_personality.get_help_intro()
        return """{intro}
...
💡 **نصيحة أزاد:** كل التقارير قابلة للتصدير إلى Excel و PDF! 📑"""
    ...
    def _show_system_quick_links():
        ...
        _ = azad_personality.get_greeting()
        return """{intro}
...
💡 **نصيحة أزاد:** احفظ الروابط المهمة في مفضلتك! 📌"""
    ...
    def _handle_suppliers_query(message):
        ...
        try:
            ...
            # موردين موثوقين
            _ = Supplier.query.filter_by(is_active=True, is_verified=True).count()

            # أفضل مورد (حسب التقييم)
            top_supplier = Supplier.query.filter_by(is_active=True).order_by(Supplier.rating.desc()).first()

            # إجمالي المشتريات
            _ = db.session.query(db.func.sum(Supplier.total_purchases_aed)).scalar() or 0

            response = """🏪 **نظام إدارة الموردين** ⭐ (نظام جديد!)
...
❌ ماء يطل من الصالون → بلف مسدود"""
    ...
    def _get_dtc_info(message):
        ...
        common_codes = {
            'P0300': '🔴 **P0300 - Misfire عام في المحرك**\n\n**السبب:**\n• بواجي قديمة\n• كويلات ضعيفة\n• بخاخات مسدودة\n• ضغط منخفض\n\n**الحل:** فحص البواجي والكويلات أولاً',  # noqa: E501
