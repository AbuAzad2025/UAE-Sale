...
class AdvancedFinancialAnalytics:
    ...
    def _calculate_balance_by_prefix(prefix, date_from=None, date_to=None, is_pl=False):
        ...
        """
        query = GLAccount.query.filter(GLAccount.is_active == True, GLAccount.is_header == False)  # noqa: E712
    ...
    def get_forecasting_data(months_ahead=6):
        ...
        _ = sum(item['revenue'] for item in historical) / len(historical)
        _ = sum(item['expenses'] for item in historical) / len(historical)
